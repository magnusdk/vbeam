...
class IrregularSampledCoordinate(Coordinate):
    ...
    def __post_init__(self):
        if self.x_data.ndim > 1 and self.dim is None:
            ValueError(
                "For x_data.ndim>1, self.dim need to be set as interpolation axis. got self.dim={self.dim}"
            )
        if self.x_data.ndim == 1 and self.dim is None:
    ...
    def get_nearest_indices(self, x: float, n_samples: int) -> IndicesInfo:
        ...
        indices_outside = ops.where(
            indices_around_x_clipped != indices_around_x, True, False
        )

        # indices_positions[indices_outside] += 1
        indices_positions = ops.where(
            indices_outside, indices_positions + 1, indices_positions
        )
